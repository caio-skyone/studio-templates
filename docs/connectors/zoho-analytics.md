...
- **`CONFIG`** — o parâmetro que carrega a configuração de praticamente toda chamada, como um JSON. Ele vai sempre na query string, sob a chave `CONFIG`, em qualquer método. É o que substitui a longa lista de parâmetros que outras APIs usariam.
...
- **`CONFIG` na query (`config`).** Toda operação que precisa de configuração declara o parâmetro `config`, um JSON que o Studio envia na query string sob a chave `CONFIG`, em qualquer método (`GET`, `POST`, `PUT`, `DELETE`). Cada operação traz um exemplo real extraído da especificação oficial; use-o como ponto de partida.
- **Duas operações usam JSON puro.** **Data - Sort Data by Columns** e **Views - Auto Analyse View** recebem o corpo em `application/json`, sem `CONFIG`. O `Content-Type` correspondente já está declarado nelas.
...
- **Um único parâmetro de configuração por operação.** Como a API concentra tudo em `CONFIG`, os campos não são parametrizados individualmente: o `config` é um texto único. A descrição e o exemplo de cada operação mostram a estrutura esperada; os campos opcionais estão na documentação oficial de cada endpoint.
